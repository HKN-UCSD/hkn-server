...
export class changePointsRequirement1635072472752 implements MigrationInterface {
  name = 'changePointsRequirement1635072472752';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE VIEW "inductee_points_view" AS SELECT "appUser"."id" AS "user", "appUser"."email" AS "email", SUM("attendance"."points") AS "points", CASE WHEN( SUM(CASE WHEN "event"."type" = 'professional' THEN "attendance"."points" ELSE 0 END)::float ) >= 1 THEN TRUE ELSE FALSE END AS "hasProfessionalRequirement", CASE WHEN( SUM(CASE WHEN "event"."type" = 'mentorship' THEN "attendance"."points" ELSE 0 END)::float ) >= 1 THEN TRUE ELSE FALSE END AS "hasMentorshipRequirement", CASE WHEN( SUM(CASE WHEN "event"."type" = 'technical' THEN "attendance"."points" ELSE 0 END)::float ) >= 1 THEN TRUE ELSE FALSE END AS "hasTechnicalRequirement", CASE WHEN( SUM(CASE WHEN "event"."type" = 'social' THEN "attendance"."points" ELSE 0 END)::float ) >= 2 THEN TRUE ELSE FALSE END AS "hasSocialRequirement" FROM "app_user" "appUser" INNER JOIN "attendance" "attendance" ON "appUser"."id" = "attendance"."attendeeId"  INNER JOIN "event" "event" ON "event"."id" = "attendance"."eventId" WHERE "attendance"."isInductee" GROUP BY "appUser"."id"`
    );
    await queryRunner.query(
